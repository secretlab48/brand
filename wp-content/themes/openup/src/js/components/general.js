import {Popup, Cookie} from "./modules";
...
// console.log(cookie.getCookie('preferred_lang'));
    ...
    $.getJSON( 'https://api.ipregistry.co/?key=8d5s163eq3ltpuwo&fields=connection,location.country.code&hostname=true&output=xml&pretty=true' )
        .done( function( data ) {
            const countryCode = (typeof data.location != 'undefined') ? data.location.country.code : 'UA';
...
if(document.querySelector('.JS-check-href') != null) {
  let sectionLink = document.querySelectorAll('.JS-check-href');
